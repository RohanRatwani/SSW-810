...
class datetestclass(unittest.TestCase):
    ...
    def test_analyze_files(self):
        ...
        output: Dict = {'0_defs_in_this_file.py': {'classes': 0, 'functions': 0, 'lines': 3, 'chars': 55},
                        'file1.py': {'classes': 2, 'functions': 4, 'lines': 25, 'chars': 270},
                        'file2.py': {'classes': 2, 'functions': 5, 'lines': 20, 'chars': 205}}
        self.assertEqual(f.analyze_files(), output)

    def test_file_reader(self):
        """The function tests the file_reader() function."""
        a: List[tuple] = [('123', 'Jin He', 'Computer Science'), ('234', 'Nanda Koka', 'Software Engineering')]
        self.assertEqual(list(file_reader("test.txt", 3, '|', True)), a)
