...
def file_reader(path: str, fields: int, sep: str = '|', header: bool = False) -> Iterator[Tuple[str]]:
    #This function is for Reading text files with a fixed number of fields, separated by a specific character
    line_num: int = 0
    try:
        fp = open(path, 'r')
        ...
        with fp:
            ...
            for line in fp:
                ...
                try:
                    ...
                    yield ( f"There's an error in File Name: {path}, Line number: {line_num}, Number of fields found in Line: {len(line)}, Expected number of fields: {fields}")
                else:
                    yield item
'''
