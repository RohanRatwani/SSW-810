...
def file_reader(path:str, fields, sep=',', header=False) -> Iterator[Tuple[str]]:
    ...
    try:
        ...
        with fp:
            ...
            for line in fp: 
                line1 = line.replace("\n"," ")
                item = tuple(line1.split("|"))
                count += 1
                try:
                    if len(item) != fields:
                        raise ValueError
                except ValueError:
                    yield(f"Value error: {path} has {fields} on line {count} but expected {len(item)}")
                else:  
                    yield item
class FileAnalyzer:
...
file_analyzer:FileAnalyzer = FileAnalyzer("R:\Stevens\Sem-2\SSW-810\HW_08_Rohan_Ratwani\Ronnie")
